...
> [!warning] 
>  应注意一个问题，git 初始化之后是自动会生成一个 master 的，但是此时的 master 是空的，因此需要通过 add 命令添加文件并 commit 之后，才能创建其他的分支。
...
第一次推送之后，会绑定本地和远程分支，后续直接通过 push 就可以了
...
参考 [211021_PyQt 环境配置](../../Work/211001_PyQt项目/211021_PyQt环境配置.md)
...
如果打开终端不运行，请参考 ZSH 配置中把.zshrc 中加入环境变量
...
> [!warning] 
> 安装完 OMZsh 之后一定要更新一下 zshrc 的环境变量
