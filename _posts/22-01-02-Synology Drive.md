...
> 开启 SSl 数据传输加密会极大增加 NAS 的 CPU 负载，减小传输速度，因此不推荐开启。
...
> 按需同步是指只是同步 NAS 上的文件目录，而非同步文件本体，不适用于本指南所描述的工作环境。
...
- 在**全局设置-常规**中选择**本地删除的文件将从您的 NAS 上再次下载**
